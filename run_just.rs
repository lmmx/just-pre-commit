...
fn find_just_locally() -> Option<PathBuf> {
    // Try prek first, then fall back to pre-commit
    let cache_names = ["prek", "pre-commit"];

    for name in cache_names {
        if let Some(cache_dir) = get_cache_dir(name) {
            eprintln!("Looking in cache directory: {}", cache_dir.display());

            if cache_dir.exists() {
                if let Some(path) = find_just_in_dir(&cache_dir) {
                    return Some(path);
                }
            }
        }
...
fn find_just_in_dir(dir: &PathBuf) -> Option<PathBuf> {
    let output = Command::new("find")
        .arg(dir)
        .arg("-name")
        .arg("just")
